...
public class BoardController implements Controller {
	...
	public String execute(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		//jsp - 입력 - action(insert) - 1 - 성공 - action(select) - jsp
			...
			int result = 0;
			// hmb.bind(pMap);
			// post 방식은 매번 동일한 url을 요청하더라도 무조건 서버를 경유한다
			// get방식은 같은 url이면 인터셉트 당함
			hmb.multiBind(pMap); // 첨부파일이 포함된 post 방식의 처리일 때만 - get방식은 무조건 첨부파일 안 됨
...
// path에 담겨있어서 res.sendRedirect("/"+path+".jsp") -> http://localhost:8000/board/boardList.gd2.jsp
