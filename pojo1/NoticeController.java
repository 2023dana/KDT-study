...
public class NoticeController implements Action {
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		////////////////////<< 사용자로 부터 입력받은 값 처리 >>///////////////////
		if("noticeList".equals(upmu[1])) {//select - 1-3버전에서는 이 장면을 메소드 단위로 변경하고 싶다(req, res)넘겨 받을 수 있어야 한다. -문제 해결못하니까
		...
		//jsp - 입력 - action(insert) - 1 - 성공 - action(select) - jsp
			...
			int end = path.toString().length();// -> notice/
			path.delete(0, end);
			path.append(temp);//url이 전달되는게 아니라 json형식 즉 문자열이 전달됨  - avatar.png
		}// end of imageUpload
		//http://localhost:8000/notice/imageGet.gd?imageName=avatar25.png
			...
			String filePath = "D:\\workspace_jsp\\nae2Gym\\src\\main\\webapp\\pds" ;
