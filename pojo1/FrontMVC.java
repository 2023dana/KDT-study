...
public class FrontMVC extends HttpServlet {
	...
	protected void doService(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		//////////////////////[[ 컨트롤을 타고 난 뒤에 내가 할일은? ]]///////////////////////
		//ViewResolver클래스가 추가될 코드 블록 이다. - 1-3개선해 본다 -> if문으로 분기되는 걸 클래스 설계로 변경해서 처리해 본다.(의의)
		...
		//문제제기  - 여러가지 타입을 고려하다보니 if문이 중첩되었다 - 별로다 - 1-3버전에서는 개선해 본다 -> spring ViewResolver클래스로 지원하고 있다.이미
		if(af !=null) {
			if(af.isRedirect()) {//true라는 건 sendRedirect인 경우임
				...
				//3. null일때 - 이미지 업로드 처리시나 첨부파일 처리시에는 리턴으로 나갈 값이 필요없다.
					...
					//jsp-htmll,css,js,java, <%%>, 혼재되어 있다. 정적페이지와 동적페이지가 혼재
					//ReactJS - 지원하는 방법 - @RestController
					res.setContentType("text/plain;utf-8");//@RestController or @ResponseBody
